...
open class LandscapeBatteryDrawableiOS16(private val context: Context, frameColor: Int) : Drawable(), BatteryDrawable {
    ...
    private val IOS_GREEN = 0xFF34C759.toInt()
    private val IOS_POWERSAVE = 0xFFFFCC0A.toInt()
    ...
    private fun batteryColorForLevel(level: Int): Int {
        return when {
            charging -> IOS_GREEN
            powerSaveEnabled -> IOS_POWERSAVE
    ...
    private fun loadPaths() {
        val pathString = context.resources.getString(
                R.string.LandPerimeterPathiOS16)
        perimeterPath.set(PathParser.createPathFromPathData(pathString))
        perimeterPath.computeBounds(RectF(), true)

        errorPerimeterPath.set(PathParser.createPathFromPathData(pathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        fillMask.set(PathParser.createPathFromPathData(pathString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val boltPathString = context.resources.getString(
                R.string.LandBoltPathiOS16)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        plusPath.set(PathParser.createPathFromPathData(pathString))
